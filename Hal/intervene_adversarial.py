...
def formatter(model_name, dataset_name, dataset, tokenizer): 
    ...
    for index, row in enumerate(dataset):
        if dataset_name in ['truthfulqa', 'triviaqa']:
            prompt = tokenizer(format_prompt(base_prompt.format(question=row['question'])), return_tensors='pt')
        elif dataset_name == 'tydiqa':
            prompt = tokenizer(format_prompt(base_prompt.format(context=row['context'], question=row['question'])), return_tensors='pt')
        elif dataset_name == 'coqa':
            prompt = tokenizer(format_prompt(base_prompt.format(story=row['story'], question=row['question'])), return_tensors='pt')
        elif dataset_name == 'haluevaldia':
            prompt = tokenizer(format_prompt(base_prompt.format(knowledge=row['knowledge'], dialogue_history=row['dialogue_history'])), return_tensors='pt')
        elif dataset_name == 'haluevalqa':
            prompt = tokenizer(format_prompt(base_prompt.format(context=row['knowledge'], question=row['question'])), return_tensors='pt')
        elif dataset_name == 'haluevalsum':
            prompt = tokenizer(format_prompt(base_prompt.format(document=row['document'])), return_tensors='pt')
        all_prompts.append(prompt)
        
    return all_prompts

def run_answers(test_idxs, model_name, dataset_name, questions, dataset, tokenizer=None, model=None,
                 verbose=False, device=DEVICE, instruction_prompt="default"):
...
def main(): 
    ...
    # head_wise_activations_ = head_wise_activations[perm]
    # labels = labels[perm]
    ...
    for i in range(args.num_fold):
        ...
        filename = f'{args.dataset_name}_{args.model_name}_top{args.num_layers}layers_alpha{args.alpha}_recon{args.lambda_recon}_advgen'
