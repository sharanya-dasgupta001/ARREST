...
def alt_evaluate(questions, dataset, tokenizer, test_idxs, model_name, dataset_name, models, output_path, device=DEVICE,
    ...
    for mdl in models.keys(): 
        ...
        answers = run_answers(test_idxs, model_name, dataset_name, questions, dataset, model=llama_model, tokenizer=tokenizer,
...
class ISI_Intervener():
    ...
    collect_action = True
    attr_idx = -1
    def __init__(self, generator, discriminator, multiplier=1):
        self.multiplier = multiplier
    ...
    def __call__(self, b, s): 
        ...
        self.discriminator.to(b.dtype).to(b.device)
        self.generator.eval()
        self.discriminator.eval()
        with torch.no_grad():
            b[0, -1] = self.multiplier * self.generator(b[0, -1]) + (1 - self.multiplier) * b[0, -1]
        return b
...
def main(): 
    ...
    parser.add_argument('--alpha1', type=float, default=15, help='alpha, intervention strength')
    parser.add_argument('--alpha2', type=float, default=0.5, help='alpha, intervention strength')
    ...
    #     head_wise_activations.append(np.load(f"/home/iplab/LLM/mitigation_results/{model_name}_{dataset_name}_layer_wise.npy"))
    ...
    # labels_hal = np.load(f"/home/iplab/LLM/mitigation_results/{args.model_name}/{args.model_name}_{args.dataset_name}_labels.npy")
    # labels_truth = np.load(f"/home/iplab/LLM/mitigation_results/{args.model_name}_instruct/{args.model_name}_instruct_{args.dataset_name}_labels.npy")
    labels_hal = np.ones(head_wise_activations_hal.shape[0])
    labels_truth = np.zeros(head_wise_activations_truth.shape[0])
    head_wise_activations_base_iti = rearrange(head_wise_activations_hal, 'b l (h d) -> b l h d', h = num_heads)
    head_wise_activations_instruct_iti = rearrange(head_wise_activations_truth, 'b l (h d) -> b l h d', h = num_heads)
    ...
    for i in range(args.num_fold):
        ...
        head_wise_activations_iti = np.concatenate([head_wise_activations_base_iti[train_idxs], head_wise_activations_instruct_iti[train_idxs]], axis=0)
        ...
        perm = np.random.permutation(len(labels))
        head_wise_activations_iti = head_wise_activations_iti[perm]
        head_wise_activations = head_wise_activations[perm]
        labels = labels[perm]
        ...
        # else:
        #     com_directions = None
        top_heads, probes = get_top_heads(train_set_idxs, val_set_idxs, head_wise_activations_iti, labels, num_layers,
        ...
        for layer, heads in top_heads_by_layer.items():
            ...
            for head in heads:
                ...
                # print(dir.shape)
                activations = torch.tensor(head_wise_activations_iti[:,layer,head,:], dtype=torch.float32).to("cpu") # batch x 128
                proj_vals = activations @ dir.T
                proj_val_std = torch.std(proj_vals)
                direction[head * head_dim: (head + 1) * head_dim] = dir * proj_val_std
            intervener = ITI_Intervener(direction, args.alpha1) #head=-1 to collect all head activations, multiplier doens't matter
        ...
        top_layers, probes = get_top_layers(train_set_idxs, val_set_idxs, head_wise_activations, labels, num_layers,
                                        num_heads, args.seed, args.num_layers, args.use_random_dir)

        print("Layers intervened: ", sorted(top_layers))

        for layer in top_layers :
            # head_wise_activations = head_wise_activations_[:,layer,:]
            # real_indices = np.where(labels == 1)[0]
            # fake_indices = np.where(labels == 0)[0]
            # real_activations = head_wise_activations[real_indices]
            # fake_activations = head_wise_activations[fake_indices]
            real_activations = head_wise_activations_hal[train_idxs,layer,:]
            fake_activations = head_wise_activations_truth[train_idxs,layer,:]
            ...
            intervener = ISI_Intervener(generator, discriminator, args.alpha2) #head=-1 to collect all head activations, multiplier doens't matter
            interveners.append(intervener)
            pv_config.append({
                "component": f"model.layers[{layer}].self_attn.o_proj.input",
                "intervention": wrapper(intervener),
        ...
        filename = f'{args.dataset_name}_{args.model_name}_top{args.num_heads}heads_top{args.num_layers}layers_alpha1{args.alpha1}_alpha2{args.alpha2}_adviti'
...
if __name__ == "__main__":
    ...
    df_results.to_csv(f"/home/iplab/LLM/mitigation_results/responses_hal/{filename}.csv", index=False)