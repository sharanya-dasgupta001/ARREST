...
def run_answers(test_idxs, model_name, dataset_name, dataset,  model=None,
...
def alt_evaluate(questions, test_idxs, model_name,dataset_name, models, output_path, device=DEVICE,
                    verbose=False, instruction_prompt="default"): 
    for mdl in models.keys(): 
        llama_model = models[mdl]
        llama_tokenizer = AutoTokenizer.from_pretrained(HF_NAMES[mdl])
        answers = run_answers(test_idxs, model_name, dataset_name, questions, model=llama_model, tokenizer=llama_tokenizer,
                        device=device, verbose=verbose,
                        instruction_prompt=instruction_prompt)

        answers.to_csv(output_path)
...
def main(): 
    ...
    parser.add_argument('--num_heads', type=int, default=3, help='K, number of top heads to intervene on')
    ...
    head_wise_activations_base = np.load(f"/home/iplab/LLM/mitigation_results/{args.model_name}/{args.model_name}_{args.dataset_name}_layer_wise.npy")
    head_wise_activations_instruct = np.load(f"/home/iplab/LLM/mitigation_results/{args.model_name}_instruct/{args.model_name}_instruct_{args.dataset_name}_layer_wise.npy")
    ...
    for i in range(args.num_fold):
        ...
        filename = f'{args.dataset_name}_{args.model_name}_heads_alpha_{args.alpha}_adversarial_layer_wise'
