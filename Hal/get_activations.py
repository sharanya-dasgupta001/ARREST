...
def formatter(model_name, dataset_name, dataset, tokenizer): 
    ...
    base_prompts = {
        ...
        'haluevalsum': "{document} \n Please summarize the above article concisely. A: {answer}",
    ...
    for index, row in enumerate(dataset):
        if dataset_name in ['truthfulqa']:
            prompt = tokenizer(format_prompt(base_prompt.format(question=row['question'], answer=row['best_answer'])), return_tensors='pt').input_ids
        if dataset_name in ['triviaqa']:
            prompt = tokenizer(format_prompt(base_prompt.format(question=row['question'], answer=row['answer'])), return_tensors='pt').input_ids
        elif dataset_name == 'tydiqa':
            prompt = tokenizer(format_prompt(base_prompt.format(context=row['context'], question=row['question'], answer=row['answers'])), return_tensors='pt').input_ids
        elif dataset_name == 'coqa':
            prompt = tokenizer(format_prompt(base_prompt.format(story=row['story'], question=row['question'], answer=row['answer'])), return_tensors='pt').input_ids
        elif dataset_name == 'haluevaldia':
            prompt = tokenizer(format_prompt(base_prompt.format(knowledge=row['knowledge'], dialogue_history=row['dialogue_history'], answer=row['right_response'])), return_tensors='pt').input_ids
        elif dataset_name == 'haluevalqa':
            prompt = tokenizer(format_prompt(base_prompt.format(context=row['knowledge'], question=row['question'], answer=row['right_answer'])), return_tensors='pt').input_ids
        elif dataset_name == 'haluevalsum':
            prompt = tokenizer(format_prompt(base_prompt.format(document=row['document'], answer=row['right_summary'])), return_tensors='pt').input_ids
        ##########################################################################################################################################################
...
def main(): 
    ...
    np.save(f'/home/iplab/LLM/mitigation_results/{args.model_name}/{args.model_name}_{args.dataset_name}_layer_wise_truthful.npy', all_layer_wise_activations)
    
    print("Saving head wise activations")
    np.save(f'/home/iplab/LLM/mitigation_results/{args.model_name}/{args.model_name}_{args.dataset_name}_head_wise_truthful.npy', all_head_wise_activations)
