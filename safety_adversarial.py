...
from utils import ASREvaluator, Generator, Discriminator, Intervener_safety_adv
import pyvene as pv
...
def formatter(dataset_name, dataset, tokenizer): 
    """Formats dataset prompts for tokenization."""
    base_prompts = {
...
def get_answers(i, test_idxs, dataset_name, questions, dataset,  model=None,
    ...
    """Generates model responses for a subset of data."""
    ...
    # Prepare final frame with relevant columns
    if dataset_name in ['malicious-instruct','advbench','trustllm']:
...
def evaluation(i, questions, dataset, tokenizer, test_idxs, dataset_name, models, device=DEVICE, verbose=False): 
    """Evaluates the LLMs after intervention."""
...
def get_top_layers(i, train_idxs, val_idxs, activations, labels, num_layers,
    ...
    """Returns top layers based on probe accuracy."""
...
def main(): 
    """Main pipeline for training and evaluating ARREST-Adversarial."""
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_name', type=str, default='llama2_7B', choices=HF_NAMES.keys(), help='model name')
    parser.add_argument('--dataset_name', type=str, default='malicious-instruct', help='Dataset', choices=['malicious-instruct', 'advbench', 'jailbreak-bench', 'trustllm'])
    ...
    print(f"""\n
    ==================================================================================
                  ARREST-Adversarial for Safety Execution Started
    ==================================================================================
    ...
    for i in range(args.num_fold):
        ...
        for layer in top_layers :
            ...
            train_dataset = TensorDataset(torch.tensor(base_activations), torch.tensor(aligned_activations))
            dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
            generator, discriminator = train_adversarial(i+1, hidden_size, dataloader, num_epochs=args.num_epochs, lr=args.lr, lambda_recon=args.lambda_recon, device=DEVICE)
            intervener = Intervener_safety_adv(generator, discriminator, args.alpha) 
            pv_config.append({
        ...
        filename = f'{args.dataset_name}_{args.model_name}_top{args.num_layers}layers_alpha{args.alpha}_adv'                  
        tqdm.write('')
        evaluation(
            i+1,
            prompts,
